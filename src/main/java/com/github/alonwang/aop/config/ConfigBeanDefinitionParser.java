...
import com.github.alonwang.beans.factory.support.BeanDefinitionReaderUtils;
...
public class ConfigBeanDefinitionParser /* implements BeanDefinitionParser */ {
	...
	public BeanDefinition parse(Element element,
		...
		for (Element elt : childElts) {
			...
			/*
			 * if (POINTCUT.equals(localName)) { parsePointcut(elt, registry); }
			 */
			/*
			 * else if (ADVISOR.equals(localName)) { parseAdvisor(elt,
			 * registry); }
			 */
		...
		for (int i = 0; i < eleList.size(); i++) {
			...
			if (isAdviceNode(ele)) {
				...
				GenericBeanDefinition advisorDefinition = parseAdvice(
						aspectName, i, aspectElement, ele, registry,
	...
	/**
	 * Return {@code true} if the supplied node describes an advice type. May be
	 * one of: '{@code before}', '{@code after}', '{@code after-returning}',
	...
	private GenericBeanDefinition parseAdvice(String aspectName, int order,
			Element aspectElement, Element adviceElement,
			BeanDefinitionRegistry registry,
			List<BeanDefinition> beanDefinitions,
			List<RuntimeBeanReference> beanReferences) {

		GenericBeanDefinition methodDefinition = new GenericBeanDefinition(
		...
		GenericBeanDefinition aspectFactoryDef = new GenericBeanDefinition(
				AspectInstanceFactory.class);
		aspectFactoryDef.getPropertyValues()
				.add(new PropertyValue("aspectBeanName", aspectName));
		aspectFactoryDef.setSynthetic(true);

		// register the pointcut
		GenericBeanDefinition adviceDef = createAdviceDefinition(adviceElement,
				registry, aspectName, order, methodDefinition, aspectFactoryDef,
				beanDefinitions, beanReferences);

		adviceDef.setSynthetic(true);

		// register the final advisor
		BeanDefinitionReaderUtils.registerWithGeneratedName(adviceDef,
				registry);
	...
	/**
	 * Creates the RootBeanDefinition for a POJO advice bean. Also causes
	 * pointcut parsing to occur so that the pointcut may be associate with the
	 * advice bean. This same pointcut is also configured as the pointcut for
	 * the enclosing Advisor definition using the supplied
	 * MutablePropertyValues.
	 */
	private GenericBeanDefinition createAdviceDefinition(Element adviceElement,
			BeanDefinitionRegistry registry, String aspectName, int order,
			GenericBeanDefinition methodDef,
			GenericBeanDefinition aspectFactoryDef,
			List<BeanDefinition> beanDefinitions,
			List<RuntimeBeanReference> beanReferences) {

		GenericBeanDefinition adviceDefinition = new GenericBeanDefinition(
				getAdviceClass(adviceElement));
		adviceDefinition.getPropertyValues()
				.add(new PropertyValue(ASPECT_NAME_PROPERTY, aspectName));

		ConstructorArgument cav = adviceDefinition.getConstructArgument();
		...
		if (pointcut instanceof BeanDefinition) {
			...
			RuntimeBeanReference pointcutRef = new RuntimeBeanReference(
					(String) pointcut);
	...
	/**
	 * Gets the advice implementation class corresponding to the supplied
	 * {@link Element}.
	 */
	private Class<?> getAdviceClass(Element adviceElement) {
		...
		/*
		 * else if (AFTER.equals(elementName)) { return
		...
		/*
		 * else if (AROUND.equals(elementName)) { return
	 ...
	 */
	private GenericBeanDefinition parsePointcut(Element pointcutElement,
			BeanDefinitionRegistry registry) {
		...
		if (StringUtils.hasText(pointcutBeanName)) {
			registry.registerBeanDefinition(pointcutBeanName,
					pointcutDefinition);
		} else {
			BeanDefinitionReaderUtils
					.registerWithGeneratedName(pointcutDefinition, registry);
	...
	/**
	 * Parses the {@code pointcut} or {@code pointcut-ref} attributes of the
	 * supplied {@link Element} and add a {@code pointcut} property as
	 * appropriate. Generates a
	 * {@link org.springframework.beans.factory.config.BeanDefinition} for the
	 * pointcut if necessary and returns its bean name, otherwise returns the
	 * bean name of the referred pointcut.
	 */
	private Object parsePointcutProperty(
			Element element/* , ParserContext parserContext */) {
		if ((element.attribute(POINTCUT) == null)
				&& (element.attribute(POINTCUT_REF) == null)) {
			/*
			...
			GenericBeanDefinition pointcutDefinition = createPointcutDefinition(
					expression);
			...
			if (!StringUtils.hasText(pointcutRef)) {
				/*
				 * parserContext.getReaderContext().error(
				 * "'pointcut-ref' attribute contains empty value.", element,
	...
	/**
	 * Creates a {@link BeanDefinition} for the
	 * {@link AspectJExpressionPointcut} class using the supplied pointcut
	 * expression.
	 */
	protected GenericBeanDefinition createPointcutDefinition(
			String expression) {
		...
		beanDefinition.getPropertyValues()
				.add(new PropertyValue(EXPRESSION, expression));
